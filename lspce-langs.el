...
(defun lspce-ra-initializationOptions ()
  (let ((options (make-hash-table :test #'equal)))
    ...
    (lspce--add-option "enableExperimental" :json-false options)
    (lspce--add-option "cargo.features" "all" options)
    (lspce--add-option "cargo.noDefaultFeatures" :json-false options)
    (lspce--add-option "cargo.runBuildScripts" t options)
    (lspce--add-option "cargo.loadOutDirsFromCheck" t options)
...
(defun lspce-jdtls-initializationOptions ()
  (let ((options (make-hash-table :test #'equal)))
    ...
    (lspce--add-option "settings.java.import.gradle.offline.enabled" t options)
